...
type PopoverFormProps = {
  ...
  title: string
  /** Wrapper classes. Defaults to the standalone centred layout. */
  className?: string
  /** Extra panel classes — use to anchor the panel to the trigger. */
  panelClassName?: string
  /** Trigger classes. Defaults to a bordered label button. */
  triggerClassName?: string
  /** Trigger content. Defaults to the `title`, which morphs into the panel. */
  triggerChild?: ReactNode
  /** Accessible name — required when `triggerChild` is icon-only. */
  triggerLabel?: string
  /**
   * Trigger corner radius, in px. Set inline rather than by class because the
   * layout morph interpolates it; match it to the trigger's design-system radius.
   */
...
export function PopoverForm({
  ...
  return (
    ...
    >
      ...
      >
        {triggerChild ?? <motion.span layoutId={`${title}-title`}>{title}</motion.span>}
      </motion.button>
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId={`${title}-wrapper`}
            className={`bg-muted absolute overflow-hidden p-1 shadow-[0_0_0_1px_rgba(0,0,0,0.08),0px_1px_2px_rgba(0,0,0,0.04)] outline-none dark:bg-neutral-800 ${panelClassName ?? ''}`}
            ref={ref}
...
export function PopoverFormButton({
  ...
  return (
    <button
      type="submit"
      className="from-primary/90 to-primary text-primary-foreground ml-auto flex h-6 w-26 items-center justify-center overflow-hidden rounded-md bg-gradient-to-b px-3 text-xs font-semibold shadow-[0_0_1px_1px_rgba(255,255,255,0.08)_inset,0_1px_1.5px_0_rgba(0,0,0,0.32),0_0_0_0.5px_var(--primary)]"
    >
...
export function PopoverFormSeparator({
  ...
  return (
    <svg
      ...
      xmlns="http://www.w3.org/2000/svg"
    >
      <path d="M0 1H352" className="stroke-border dark:stroke-white/15" strokeDasharray="4 4" />
    </svg>
  )
}
