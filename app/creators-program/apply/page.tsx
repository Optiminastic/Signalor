...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return <label className="text-foreground mb-1.5 block text-xs font-semibold">{children}</label>
}
    ...
    <div className="relative">
      <button
        ...
        className={cn(
          ...
          'focus:ring-2 focus:ring-orange-400/40 focus:outline-none',
        )}
      >
          ...
          <div className="border-b border-black/6 px-3 py-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded border border-black/8 bg-white py-1.5 pr-2 pl-7 text-xs focus:outline-none"
              />
            </div>
          </div>
              ...
              filtered.map(c => (
                <li key={c.code}>
                  <button
                    ...
                    className={cn(
                      'hover:bg-muted flex w-full items-center gap-2 px-3 py-1.5 text-left text-[13px]',
            ...
            <button
              ...
              className={cn(
                ...
                on
                  ...
                  : 'text-muted-foreground hover:border-border border-black/12 bg-white',
              )}
            >
...
function ApplyForm({
  ...
  return (
    <form
      ...
      className="mx-auto w-full max-w-2xl rounded-2xl border border-black/8 bg-white p-6 shadow-md sm:p-8"
    >
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <Input
            ...
            value={lockedEmail}
            disabled
            className="bg-muted text-muted-foreground h-10 cursor-not-allowed border-black/12 text-[13px]"
          />
        </div>
      </div>
      ...
      <div className="bg-muted/50 mt-6 rounded-xl border border-black/8 p-4">
        ...
        <div className="mt-3">
          ...
          <textarea
            ...
            className="text-foreground placeholder:text-muted-foreground w-full resize-y rounded-md border border-black/12 bg-white px-3 py-2 text-[13px] leading-relaxed focus:ring-2 focus:ring-orange-400/40 focus:outline-none"
          />
      ...
      <Button
        type="submit"
        disabled={submitting}
        className="bg-foreground text-background mt-6 h-11 w-full gap-2 text-sm font-semibold hover:opacity-90"
      >
        {submitting ? (
...
function CopyButton({ value, label }: { value: string; label?: string }) {
  ...
  return (
    <button
      ...
      className="text-foreground hover:bg-muted inline-flex items-center gap-1 rounded-md border border-black/12 bg-white px-2.5 py-1.5 text-[11px] font-semibold"
    >
...
function SuccessCard({ result }: { result: CreatorApplyResponse }) {
  return (
    <div className="border-success/30 bg-success/40 mx-auto w-full max-w-2xl rounded-2xl border p-6 shadow-md sm:p-8">
      ...
      <div className="mt-5 space-y-3">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <code className="text-foreground rounded-md border border-black/10 bg-white px-3 py-1.5 font-mono text-sm font-semibold">
              {result.code}
            </code>
        ...
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <code className="text-foreground min-w-0 flex-1 truncate rounded-md border border-black/10 bg-white px-3 py-1.5 font-mono text-xs">
              {result.share_url}
            </code>
      ...
      <Link
        ...
        className="bg-foreground text-background mt-6 inline-flex items-center gap-1.5 rounded-md px-4 py-2.5 text-[13px] font-semibold hover:opacity-90"
      >
