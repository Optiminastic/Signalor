...
function LensOverviewPreview() {
  ...
  return (
    <div className="overflow-hidden rounded-none border border-black/8 bg-white shadow-lg">
      <div className="flex items-center justify-between border-b border-black/6 bg-muted px-5 py-3">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-b border-black/6 bg-muted/60 text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
                ...
                >
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-sm ${
                          r.you ? "font-semibold text-foreground" : "font-medium text-foreground"
                        }`}
                      >
                        {r.name}
                      </span>
                  ...
                  <td className="px-5 py-3.5 text-right text-[13px] font-medium tabular-nums text-foreground">
                    {r.mentions}
                  </td>
                  <td className="px-5 py-3.5 text-right text-[13px] font-medium tabular-nums text-foreground">
                    {r.recs}
      ...
      <div className="border-t border-black/6 bg-muted px-5 py-3">
        ...
        return (
          <article key={g.title} className="rounded-none border border-black/8 bg-white p-5 lg:p-6">
            <div className="flex items-center gap-3">
              <span
                className={`inline-flex items-center gap-1 rounded-none px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${g.tone}`}
              >
          ...
          <li key={m.title} className="grid grid-cols-1 gap-3 px-5 py-4 sm:grid-cols-[1fr_auto]">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-none bg-muted px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                  {m.vs}
                </span>
                <span className="text-[11px] font-semibold text-success">{m.impact}</span>
              </div>
              <p className="mt-2 text-sm font-semibold text-foreground">{m.title}</p>
              <p className="mt-1.5 text-xs font-light leading-relaxed text-accent-foreground">
                {m.detail}
              </p>
            </div>
      ...
      {wins.map((w) => {
        ...
        return (
          ...
          >
            ...
            <p className="mt-1.5 text-xs font-medium text-muted-foreground">{w.effort}</p>
            <p className="mt-4 inline-flex items-center gap-1.5 rounded-md bg-success/10 px-2.5 py-1 text-[11px] font-semibold text-success">
