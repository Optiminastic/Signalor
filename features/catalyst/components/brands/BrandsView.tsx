...
import { BRANDS } from '@/features/catalyst/brands-data'
...
export function BrandsView(): JSX.Element {
  ...
  const brands = BRANDS.filter(b => b.name.toLowerCase().includes(q) || b.url.includes(q))
      ...
      <header className="mb-5 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[13px] text-[var(--cat-ink-3)]">
            {BRANDS.length} client brands across your agency workspace.
          </p>
        </div>
