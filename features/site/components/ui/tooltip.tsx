...
function TooltipContent({
  ...
  return (
    <TooltipPrimitive.Portal>
      ...
      >
        {children}
        <TooltipPrimitive.Arrow className="z-50 size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px] border border-[var(--tooltip-arrow-border,transparent)] bg-[var(--tooltip-arrow,var(--color-foreground))] fill-[var(--tooltip-arrow,var(--color-foreground))]" />
      </TooltipPrimitive.Content>
    </TooltipPrimitive.Portal>
  )
}
