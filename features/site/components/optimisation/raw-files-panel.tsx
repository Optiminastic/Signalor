...
export function RawFilesPanel({ slug, pluginConnected }: Props) {
  ...
  if (error) {
    return (
      <div className="flex items-start gap-2 p-4 text-xs text-destructive dark:text-destructive">
            ...
            {isOpen ? (
              <div className="space-y-2 border-t border-border px-3 py-2">
                ...
                <textarea
                  ...
                  className="min-h-[180px] w-full resize-y rounded-md border border-border bg-background px-3 py-2 font-mono text-xs leading-relaxed text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
                  placeholder={f.default_template}
                  spellCheck={false}
                />
                {saveMutation.isError && saveMutation.variables?.name === f.name ? (
                  <p className="text-[11px] text-destructive dark:text-destructive">
                    {(saveMutation.error as Error)?.message || "Save failed."}
                  </p>
                ) : null}
                <div className="flex items-center justify-end gap-2">
                  {justSaved ? (
                    <span className="inline-flex items-center gap-1 text-[11px] text-success dark:text-success">
