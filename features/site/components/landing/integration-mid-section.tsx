      ...
      <section className="border-t border-border/60 px-6 py-20 lg:px-12">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-base font-light leading-relaxed text-muted-foreground">
            ...
            keep Signalor aligned with the systems you already trust to publish and sell.
          </p>
        </div>
        <ul className="mx-auto mt-14 grid max-w-5xl gap-6 md:grid-cols-3">
          {VALUE_ROWS.map(({ icon: Icon, title, body }) => (
            <li
              key={title}
              className="flex flex-col rounded-none border border-border bg-card p-6 text-left shadow-sm"
            >
              <span className="flex h-11 w-11 items-center justify-center rounded-none border border-border bg-muted">
