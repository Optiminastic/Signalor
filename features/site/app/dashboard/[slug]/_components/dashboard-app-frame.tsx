...
import { Menu, Moon, Sun, X } from "@/features/site/components/icons";

import { Button } from "@/features/site/components/ui/button";
import { Diamond } from "@/features/site/components/ui/intersection-diamonds";
...
export function DashboardAppFrame({
  ...
  const topBarShell = cn(
    "shrink-0 z-50 border-b border-border/40 bg-white dark:bg-foreground",
    DASHBOARD_TOPBAR_H,
  );
  ...
  return (
    <div className="flex h-dvh flex-col bg-muted/40 text-foreground dark:bg-background">
      ...
      {mobileFallback ? (
        ...
        <header className="sticky top-0 z-40 flex h-14 shrink-0 items-center gap-2 border-b border-border/40 bg-white px-3 shadow-sm backdrop-blur-md dark:bg-foreground md:hidden">
          <Button
            type="button"
            variant="ghost"
            size="icon"
