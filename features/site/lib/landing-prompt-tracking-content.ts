import { Bell, LayoutGrid, Link2, ListChecks, Radar } from '@/features/site/components/icons'
...
export const PROMPT_TRACKING_HERO = {
  titleLine1: 'Monitor AI',
  titleBadge: 'AI',
  titleIcon: Radar,
  titleLine2: 'prompts like a',
  titleAccent: 'product surface',
  subhead:
    'Track the prompts buyers ask AI models about your category, then tie every answer to citations, GEO scores, and the schema and content work your team already prioritizes.',
  footnote: 'Surface-aware views · saved run history · PDF exports',
...
export const PROMPT_TRACKING_HUB_CARDS: {
...
}[] = [
  {
    ...
    title: 'AI surfaces',
    description:
      'Group prompts by where answers appear, assistants, AI Overviews, and answer engines, so PMM and SEO chase the same buyer moments.',
    Icon: LayoutGrid,
    cta: 'Learn more',
  },
  {
...
export const PROMPT_TRACKING_PROOF_METRICS = [
  { value: '10–25', label: 'Tracked prompts / brand' },
  { value: '6', label: 'AI engines & search' },
  { value: '1-click', label: 'Live recheck' },
  { value: '1:1', label: 'Linked to GEO score' },
] as const

export const PROMPT_TRACKING_PILLAR_ROWS = [
  { label: 'Branded', value: 72, tone: 'bg-info/80' },
  { label: 'Category', value: 64, tone: 'bg-success/90' },
  { label: 'Competitive', value: 58, tone: 'bg-warning/90' },
] as const
...
export const PROMPT_TRACKING_CAPABILITY_ROWS = [
  ...
  {
    icon: ListChecks,
    title: 'Libraries & intent',
    description:
      'Organize prompts by intent, branded, category, or competitive, so weekly readouts stay legible for execs, not buried in spreadsheets.',
  },
  {
    icon: Bell,
    title: 'Drift tracking',
    description:
      'Re-run prompts on a schedule and compare against score history to see when mentions, citations, or omissions move after a launch.',
  },
  {
    icon: Link2,
...
export const PROMPT_TRACKING_FEATURES_INTRO = {
  ...
  description:
    'Generative engines summarize, cite, or skip you in full sentences. AI prompt monitoring keeps your roadmap aligned with those answers, not only with where a blue link ranked last Tuesday.',
} as const
...
export const PROMPT_TRACKING_FEATURE_CELLS: {
...
}[] = [
  {
    title: 'AI surfaces',
    description:
      'Replay the same prompt across assistants and AI overviews, see where you are cited, paraphrased, or absent before you rewrite a page.',
    mock: {
      kind: 'chat',
  ...
  },
  {
    title: 'Citation mix',
    description:
      'For each prompt set, see how often your URLs are cited, skipped, or lose to competitors, so SEO invests in the URLs models actually reuse.',
    mock: {
      kind: 'citationBar',
  ...
  },
  {
    title: 'Topic rollups',
    description:
      'Stack-rank themes, pricing objections, security questions, integrations, by share of voice inside AI answers, not just search volume.',
    mock: {
      kind: 'scoreCard',
      label: 'Coverage',
      score: '68',
      ...
      delta: '+9',
      bars: [
        { widthClass: 'w-10', tone: 'bg-info/80', label: 'Pricing prompts · 34%' },
        { widthClass: 'w-7', tone: 'bg-success/90', label: 'Security · 28%' },
        { widthClass: 'w-6', tone: 'bg-warning/90', label: 'Integrations · 22%' },
      ],
    },
  },
  {
    ...
    description:
      'Prompt-level deltas from each scheduled re-run surface drops and recoveries in one view, so PMM, SEO, and comms triage the same changes after every launch.',
    mock: {
      kind: 'alertList',
      items: [
        {
  ...
  },
  {
    ...
    description:
      'Compare mention share for the same prompt basket, built for agency and internal exec reviews, not one-off screenshots.',
    mock: {
      kind: 'competitorBars',
      ...
      rows: [
        ...
        { name: 'Acme', pct: '39%', pctNum: 39, barClass: 'bg-muted' },
        { name: 'Northwind', pct: '20%', pctNum: 20, barClass: 'bg-muted' },
      ],
    },
  },
]
...
export const PROMPT_TRACKING_WHY = {
  ...
  shipBody:
    'The surfaces your team touches weekly, from curated prompts to scheduled re-runs that tie straight into GEO execution.',
} as const
...
export const PROMPT_TRACKING_HUB_FAQ = [
  {
    ...
    answer:
      'Rank trackers optimize for blue links on a results page. AI prompt monitoring follows how assistants and AI overviews answer specific questions, whether your brand, URLs, or competitors are named, cited, paraphrased, or skipped entirely.',
  },
  {
  ...
  {
    question: 'How do I stay on top of changes?',
    answer:
      'Schedule re-analysis on a weekly or monthly cadence and compare each run against your score history to spot prompt-level drops and recoveries. Export a PDF when you need a static snapshot for email or slides.',
  },
] as const
...
export const PROMPT_TRACKING_SURFACES_FAQ = [
  ...PROMPT_TRACKING_DETAIL_FAQ,
  {
    ...
    answer:
      'Yes, mark priority surfaces inside the workspace so rollups and priority views emphasize the experiences your buyers rely on most (for example, assistant-style answers vs. overview cards).',
  },
] as const
